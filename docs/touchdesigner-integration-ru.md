...
**Режим без реального TD (разработка):** в админке включается **TouchDesigner demo mode**. Тогда сервер **не отправляет** OSC на старт/продвижение очереди и может подменять метрики на демо; для проверки очереди есть скрытая страница `/dev/queue-control` (в production API отключён без переменной окружения).
