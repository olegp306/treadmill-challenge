...
- В шаге согласий регистрации в модалке «Ознакомиться» выводится полный текст согласия на участие и обработку ПДн (отдельный модуль `consentParticipationLegalRu.ts`), оформление: Druk Wide Cyr, верхний регистр, размер текста тела 18px (константа в том же файле).
- Подпись второго пункта сокращена до «Обработка перс. данных»; карточка подстраивается под ширину строки; ряд карточек при необходимости переносится, прокрутка длинного текста в модалке удобна на iPad.
